...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			etude_create_observer( 'etude-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				etude_gutenberg_editor_init();
			} );
		} else {
		 	etude_gutenberg_editor_init();
		}
	...
	function etude_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'etude_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'etude_inited' ) ) {
	...
	// Init on page load
	function etude_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = etude_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + ETUDE_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + ETUDE_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in ETUDE_STORAGE['override_classes'] ) {
				$obj.addClass( ETUDE_STORAGE['override_classes'][i].replace( '%s', ETUDE_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( etude_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && ETUDE_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'etude_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			etude_remove_observer( 'etude-check-editor-styles-wrapper' );
			etude_create_observer( 'etude-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'etude_inited' );
	}
} );
