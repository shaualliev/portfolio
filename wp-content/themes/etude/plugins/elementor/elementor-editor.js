...
jQuery( document ).ready(
	function() {
		...
		etude_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="etude_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.etude_post_editor' );
						if ( link.length > 0 ) {
							etude_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.etude_post_editor', function(e) {
						etude_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('etude_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function etude_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
