...
if ( ! function_exists( 'etude_storage_get' ) ) {
	/**
	 ...
	 */
	function etude_storage_get( $var_name, $default = '' ) {
		global $ETUDE_STORAGE;
		return isset( $ETUDE_STORAGE[ $var_name ] ) ? $ETUDE_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'etude_storage_set' ) ) {
	/**
	 ...
	 */
	function etude_storage_set( $var_name, $value ) {
		global $ETUDE_STORAGE;
		$ETUDE_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'etude_storage_empty' ) ) {
	/**
	 ...
	 */
	function etude_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $ETUDE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $ETUDE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $ETUDE_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $ETUDE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'etude_storage_isset' ) ) {
	/**
	 ...
	 */
	function etude_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $ETUDE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $ETUDE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $ETUDE_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $ETUDE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'etude_storage_unset' ) ) {
	/**
	 ...
	 */
	function etude_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $ETUDE_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $ETUDE_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $ETUDE_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $ETUDE_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'etude_storage_inc' ) ) {
	/**
	 ...
	 */
	function etude_storage_inc( $var_name, $value = 1 ) {
		global $ETUDE_STORAGE;
		if ( empty( $ETUDE_STORAGE[ $var_name ] ) ) {
			$ETUDE_STORAGE[ $var_name ] = 0;
		}
		$ETUDE_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'etude_storage_concat' ) ) {
	/**
	 ...
	 */
	function etude_storage_concat( $var_name, $value ) {
		global $ETUDE_STORAGE;
		if ( empty( $ETUDE_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'etude_storage_get_array' ) ) {
	/**
	 ...
	 */
	function etude_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $ETUDE_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $ETUDE_STORAGE[ $var_name ][ $key ] ) ? $ETUDE_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $ETUDE_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $ETUDE_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'etude_storage_set_array' ) ) {
	/**
	 ...
	 */
	function etude_storage_set_array( $var_name, $key, $value ) {
		global $ETUDE_STORAGE;
		if ( ! isset( $ETUDE_STORAGE[ $var_name ] ) ) {
			$ETUDE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$ETUDE_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'etude_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function etude_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $ETUDE_STORAGE;
		if ( ! isset( $ETUDE_STORAGE[ $var_name ] ) ) {
			$ETUDE_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $ETUDE_STORAGE[ $var_name ][ $key ] ) ) {
			$ETUDE_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$ETUDE_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$ETUDE_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'etude_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function etude_storage_merge_array( $var_name, $key, $value ) {
		global $ETUDE_STORAGE;
		if ( ! isset( $ETUDE_STORAGE[ $var_name ] ) ) {
			$ETUDE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$ETUDE_STORAGE[ $var_name ] = array_merge( $ETUDE_STORAGE[ $var_name ], $value );
		} else {
			$ETUDE_STORAGE[ $var_name ][ $key ] = array_merge( $ETUDE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'etude_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function etude_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $ETUDE_STORAGE;
		if ( ! isset( $ETUDE_STORAGE[ $var_name ] ) ) {
			$ETUDE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			etude_array_insert_after( $ETUDE_STORAGE[ $var_name ], $after, $key );
		} else {
			etude_array_insert_after( $ETUDE_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'etude_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function etude_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $ETUDE_STORAGE;
		if ( ! isset( $ETUDE_STORAGE[ $var_name ] ) ) {
			$ETUDE_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			etude_array_insert_before( $ETUDE_STORAGE[ $var_name ], $before, $key );
		} else {
			etude_array_insert_before( $ETUDE_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'etude_storage_push_array' ) ) {
	/**
	 ...
	 */
	function etude_storage_push_array( $var_name, $key, $value ) {
		global $ETUDE_STORAGE;
		if ( ! isset( $ETUDE_STORAGE[ $var_name ] ) ) {
			$ETUDE_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $ETUDE_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $ETUDE_STORAGE[ $var_name ][ $key ] ) ) {
				$ETUDE_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $ETUDE_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'etude_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function etude_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $ETUDE_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $ETUDE_STORAGE[ $var_name ] ) && is_array( $ETUDE_STORAGE[ $var_name ] ) && count( $ETUDE_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $ETUDE_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $ETUDE_STORAGE[ $var_name ][ $key ] ) && is_array( $ETUDE_STORAGE[ $var_name ][ $key ] ) && count( $ETUDE_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $ETUDE_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'etude_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function etude_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $ETUDE_STORAGE;
		if ( ! isset( $ETUDE_STORAGE[ $var_name ] ) ) {
			$ETUDE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $ETUDE_STORAGE[ $var_name ][ $key ] ) ) {
			$ETUDE_STORAGE[ $var_name ][ $key ] = 0;
		}
		$ETUDE_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'etude_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function etude_storage_concat_array( $var_name, $key, $value ) {
		global $ETUDE_STORAGE;
		if ( ! isset( $ETUDE_STORAGE[ $var_name ] ) ) {
			$ETUDE_STORAGE[ $var_name ] = array();
		}
		if ( empty( $ETUDE_STORAGE[ $var_name ][ $key ] ) ) {
			$ETUDE_STORAGE[ $var_name ][ $key ] = '';
		}
		$ETUDE_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'etude_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function etude_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $ETUDE_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $ETUDE_STORAGE[ $var_name ] ) ? $ETUDE_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $ETUDE_STORAGE[ $var_name ] ) ? $ETUDE_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'etude_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function etude_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $ETUDE_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $ETUDE_STORAGE[ $var_name ]->$prop ) ? $ETUDE_STORAGE[ $var_name ]->$prop : $default;
	}
}
