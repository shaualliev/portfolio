<div class="front_page_section front_page_section_team<?php
	$etude_scheme = etude_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $etude_scheme ) && ! etude_is_inherit( $etude_scheme ) ) {
		echo ' scheme_' . esc_attr( $etude_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( etude_get_theme_option( 'front_page_team_paddings' ) );
	if ( etude_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$etude_css      = '';
		$etude_bg_image = etude_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$etude_anchor_icon = etude_get_theme_option( 'front_page_team_anchor_icon' );
	$etude_anchor_text = etude_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( etude_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$etude_css      = '';
			$etude_bg_mask  = etude_get_theme_option( 'front_page_team_bg_mask' );
			$etude_bg_color_type = etude_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $etude_bg_color_type ) {
				$etude_bg_color = etude_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$etude_caption = etude_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $etude_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $etude_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $etude_caption, 'etude_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$etude_description = etude_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $etude_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $etude_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $etude_description ), 'etude_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! etude_exists_trx_addons() ) {
						...
						etude_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
