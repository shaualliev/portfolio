...
if ( function_exists( 'etude_elementor_is_preview' ) && etude_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			etude_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	etude_new_main_query(
		array(
			'post_type'      => etude_get_theme_option( 'post_type' ),
			'category'       => etude_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'etude_filter_get_template_part', etude_blog_archive_get_template() ) );
}
